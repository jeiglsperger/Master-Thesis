...
def load_datasets():
    ...
    :return: raw dataset
    """
    dataset_raw = pd.read_csv("/Josef/CTGAN/Data/Schachtschneider_externals_cut.csv", sep=',', decimal='.')
    ...
    dataset_raw.name = "Schachtschneider"
    
    return dataset_raw


# the Schachtschneider dataset was already imputed in the importer algorithm
...
def preprocess_dataset(*shift_numbers):
    ...
    dataset = dataset.drop(columns=["Auf. Datum"])
    ...
    dataset_test.to_csv('/Josef/CTGAN/test_data_schachtschneider.csv', index=False)
...
def postprocess_dataset(eval_or_opt, samples, *shift_numbers):
    ...
    for column in samples.columns:
        if (column != "mean_temp" and column != "public_holiday" and column != "school_holiday" and column != "weekday" 
            and column != "Auf. Datum"):
