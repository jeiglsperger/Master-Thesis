...
def load_datasets():
    ...
    dataset_raw['weekday'] = dataset_raw['Auf. Datum'].dt.weekday
...
def real_data_loading():
    ...
    dataset_done = dataset_raw.drop(columns=["Auf. Datum"])
...
def preprocess_data(dataset, database_name, *shift_numbers):
    ...
    encoded_dataset_eval = eval_enc.fit_transform(dataset[["public_holiday", "weekday"]])
    encoded_dataset_eval = pd.DataFrame(encoded_dataset_eval, index=dataset.index)
    dataset = pd.concat([dataset, encoded_dataset_eval], axis=1).drop(["public_holiday", "weekday"], axis=1)
