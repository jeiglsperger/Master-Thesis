...
from utils.data_loader_TouristsIndia import real_data_loading, postprocess_data
...
def prepare_train_data(database_name, *shift_numbers):
    ...
    """
    x_train, y_train, dataset_val, dataset, labels, columns, shift_numbers = real_data_loading(database_name, *shift_numbers)
...
#     dataset = dataset.reshape((dataset.shape[0], 1, 1))
    ...
    dataset = dataset.reshape((dataset.shape[0], x_train.shape[1], 1))
    
    return x_train, y_train, dataset_val, dataset, labels, columns, shift_numbers


def objective(trial, x_train, y_train, columns, dataset_val, database_name, *shift_numbers):
    """
    ...
    # generate data
    generated_data = aug.discriminative_guided_warp(x_train, y_train, batch_size=batch_size, slope_constraint=slope_constraint, 
                                                    use_window=use_window, dtw_type=dtw_type, use_variable_slice=use_variable_slice)

    # postprocessing
    fake_data = postprocess_data("opt", generated_data, columns, database_name, *shift_numbers)
...
def run_DTW(n_trials=50, database_name="default", *shift_numbers):
    ...
    x_train, y_train, dataset_val, dataset, labels, columns, shift_numbers = prepare_train_data(database_name, *shift_numbers)
    
    # optimize hyperparameters
    study = optuna.create_study(direction="maximize", storage=optuna.storages.RDBStorage("sqlite:///" + database_name + ".db"))
    study.optimize(lambda trial: objective(trial, x_train, y_train, columns, dataset_val, database_name, *shift_numbers), n_trials)
    ...
    # postprocessing
    fake_data = postprocess_data("eval", generated_data, columns, database_name, *shift_numbers)
