...
from own_data_loading_schachtschneider import real_data_loading, preprocess_data, cut_data
...
def postprocess_data(eval_or_opt, generated_data, columns, num_columns_cat, seq_len, *shift_numbers):
    ...
    conditioned_dataset = []
    for i in range(np.shape(inverse_transformed_data)[0]):
        for j in range(np.shape(inverse_transformed_data)[1]):
...
def objective(trial, dataset_train, dataset_val, num_samples, columns, num_columns_cat, database_name, *shift_numbers):
    ...
    seq_len = trial.suggest_categorical("seq_len", [1, 5, 30, 365])
...
def run_TimeGAN(num_samples, n_trials, database_name, *shift_numbers):
    ...
    # postprocessing
    fake_data = postprocess_data("eval", generated_data, columns, num_columns_cat, objective.seq_len, *shift_numbers)
